...
The driver splits each ``_index.md`` at the first ``***``: the block *above*
(YAML frontmatter + the auto-generated link list, regenerated by ``wiki update``)
is taken from ours unconditionally; the block *below* (hand-written content) is a
real three-way merge via ``git merge-file``. The driver recombines
``ours_above + merged_below`` and exits with ``git merge-file``'s status.
...
def _driver() -> pathlib.Path:
    ...
    wiki owns and wires this script -- ``wiki config``/``wiki init`` configure
    git's merge driver to wiki's own copy (wiki is a hard dependency that ships
    ``_config/**`` as package data). The test exercises exactly that script,
    which wiki resolves from its package.
    """
    import wiki
...
def _init_repo_with_driver(repo: pathlib.Path) -> None:
    """Init a git repo wiring the wiki-index driver exactly as the repo does."""
    repo.mkdir(parents=True, exist_ok=True)
    _git(repo, 'init', '-b', 'main')
    _git(repo, 'config', 'user.email', 'test@test.com')
    _git(repo, 'config', 'user.name', 'Test')
    _git(repo, 'config', 'merge.wiki-index.name', 'wiki index merge')
    _git(repo, 'config', 'merge.wiki-index.driver', f"bash '{_driver()}' %A %O %B")
...
def _run_driver(
    ...
    *,
    ...
    Mirrors how git calls ``merge_index.sh %A %O %B``: ``%A`` is ours (rewritten
    in place with the result), ``%O`` base, ``%B`` theirs. Returns the exit code
    and the rewritten ``%A`` content.
    """
    a = tmp_path / 'A'
    o = tmp_path / 'O'
    b = tmp_path / 'B'
