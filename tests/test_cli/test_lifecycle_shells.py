...
``destroy.sh`` is the only one of the four with real logic -- the repo-wide
fractal teardown (``fractal destroy`` -> ``Node.destroy``). The others are
pre/post hooks whose state changes live in ``node.py``; here we pin only
their shell-level contract (accept a path, reject a missing one).
...
__all__ = [
    ...
    'test_destroy_removes_worktrees_and_branches',
...
def test_destroy_removes_worktrees_and_branches(
    tmp_path: pathlib.Path,
) -> None:
    """Destroy tears down worktrees, their branches, and ``.worktrees/``."""
    repo = _make_repo(tmp_path / 'repo')
