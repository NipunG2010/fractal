...
``Node.init()`` calls adopt the live node as parent via ``_resolve_caller``
...
def test_loop_env_neutralized() -> None:
    ...
    (``_resolve_caller``) and can spawn stray nodes in the live DB.
    """
...
def test_resolve_caller_is_none_in_hermetic_suite() -> None:
    ...
    ``Node.init()`` resolves its parent via ``_resolve_caller`` and only
    falls back to its own repo when that is ``None``. If the ambient
    ``_NODE`` leaks, the live node is adopted instead -- the root of the
    stray-node bug. ``_resolve_caller`` is read-only, so this is safe.
    """
    assert Node._resolve_caller() is None
