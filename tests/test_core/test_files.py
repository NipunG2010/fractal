"""``Node.files_*`` -- the project-files surface.
...
def _commit_iter(node: Node, run_id: int, iter_id: int, name: str) -> str:
    ...
    (node._root / name).write_text(name, encoding='utf-8')
    _git(node._root, 'add', '-A')
    _git(node._root, 'commit', '-m', name)
    sha = _git(node._root, 'rev-parse', 'HEAD').stdout.strip()
...
def _changed_names(node: Node, since: str) -> set[str]:
    ...
    return {entry['path'] for entry in node.files_list(since=since)}
...
def test_files_read_caps_content_and_enforces_the_allowlist(
    ...
    assert node.files_path('output/logo.png').read_bytes().startswith(b'\x89PNG')
...
def test_files_read_truncation_preserves_line_terminators(
    ...
    node = node_with_db
    (node._root / 'crlf.txt').write_bytes(b'# a\r\nbeta\r\n')
    _git(node._root, 'add', '-A')
    _git(node._root, 'commit', '-m', 'crlf')
    capped = node.files_read('crlf.txt', max_lines=1)
...
def test_files_list_round_trips_non_ascii_paths(node_with_db: Node) -> None:
    ...
    node = node_with_db
    name = 'déjà.md'
    (node._root / name).write_text('encore\n', encoding='utf-8')
    _git(node._root, 'add', '-A')
    _git(node._root, 'commit', '-m', 'non-ascii')
    # NUL-parsed listings return the path verbatim, never C-quoted
    assert name in {entry['path'] for entry in node.files_list()}
    assert node.files_read(name)['content'] == 'encore\n'
    assert node.files_path(name).name == name
...
def test_files_symlinks_serve_in_tree_targets_only(node_with_db: Node) -> None:
    ...
    assert node.files_read('inside.link')['content'] == 'inside\n'
    ...
    with pytest.raises(ValueError):
        node.files_read('escape.link')
    with pytest.raises(ValueError):
        node.files_path('escape.link')
...
def test_files_list_changed_is_the_diff_from_base(node_with_db: Node) -> None:
    ...
    changed = {entry['path']: entry for entry in node.files_list(since='base')}
...
def test_files_list_changed_without_an_anchor_is_empty(node_with_db: Node) -> None:
    ...
    assert node.files_list(since='base') == []
    # a node that never committed through the loop has no iteration/run
    # anchor, even once a base is configured
    node.config_set(base=_git(node._root, 'rev-parse', 'HEAD~1').stdout.strip())
    assert node.files_list(since='iteration') == []
    assert node.files_list(since='run') == []
    # an unknown scope is rejected
    with pytest.raises(ValueError):
        node.files_list(since='bogus')
...
def test_files_list_changed_since_narrows_by_commit_iteration_run(
    ...
    r1 = node.run_start()
    _commit_iter(node, r1, node.iter_start(run_id=r1, iter=1), 'a.txt')
    _commit_iter(node, r1, node.iter_start(run_id=r1, iter=2), 'b.txt')
    r2 = node.run_start()
    _commit_iter(node, r2, node.iter_start(run_id=r2, iter=1), 'c.txt')
...
def test_files_list_changed_since_ignores_other_nodes_history(
    ...
    r2 = peer.run_start()
    _commit_iter(peer, r2, peer.iter_start(run_id=r2, iter=1), 'peer.txt')
...
def test_files_list_base_covers_uploads_before_the_first_loop_commit(
    ...
    node = node_with_db
    fork = _git(node._root, 'rev-parse', 'HEAD').stdout.strip()
    # the fork sha init.sh stamps at branch creation
    node.event_start('init', metadata=fork)
    # an upload committed before the loop ever ran (no commit event -- a
    # first-commit-event anchor would silently exclude it)
    node.files_write('inputs/data.csv', b'a,b\n1,2\n')
    node.files_commit(['inputs/data.csv'], 'seed inputs')
    r1 = node.run_start()
    _commit_iter(node, r1, node.iter_start(run_id=r1, iter=1), 'a.txt')
...
def test_diff_anchors_pin_to_the_current_incarnation(node_with_db: Node) -> None:
    ...
    node.event_start('init', metadata=fork)
    r1 = node.run_start()
    sha = _commit_iter(node, r1, node.iter_start(run_id=r1, iter=1), 'a.txt')
    # the re-init: a fresh init event stamps the new fork point (the tip)
    node.event_start('init', metadata=sha)
...
def test_files_read_before_serves_both_sides_of_the_diff(node_with_db: Node) -> None:
    ...
    assert node.files_read('mod.md', since='base', before=True) == {
        'path': 'mod.md',
        'content': 'before\n',
        'truncated': False,
        'total_lines': 1,
    ...
    assert node.files_read('new.md', since='base', before=True)['exists'] is False
    assert node.files_read('new.md')['content'] == 'fresh\n'
    # deleted: before preserved, no current side
    assert node.files_read('del.md', since='base', before=True)['content'] == 'bye\n'
    assert node.files_read('del.md', since='base')['exists'] is False
    # a before read requires an explicit anchor
    with pytest.raises(ValueError):
        node.files_read('mod.md', before=True)
...
def test_files_write_lands_in_worktree_and_rejects_escapes(
    ...
    """``files_write`` puts bytes in the worktree; escapes are rejected.
    ...
    node.files_commit(['inputs/data.csv'], 'add input')
...
def test_files_commit_commits_only_the_named_paths(node_with_db: Node) -> None:
    """``files_commit`` stages and commits just the named paths (pathspec).
    ...
    result = node.files_commit(['inputs/keep.txt'], 'add keep')
    ...
    assert node.files_commit(['inputs/keep.txt'], 'again') == {
        'committed': False,
        'sha': None,
    ...
    with pytest.raises(ValueError):
        node.files_commit([], 'msg')
    with pytest.raises(ValueError):
        node.files_commit(['inputs/keep.txt'], '')
    for bad in ('../escape', '.fractal/x', '.git/x', '*'):
        with pytest.raises(ValueError):
            node.files_commit([bad], 'msg')
...
def test_files_writes_refuse_on_a_paused_node(node_with_db: Node) -> None:
    ...
    assert node.files_read('output/REPORT.md')['content'] == _REPORT
    assert 'src/main.py' in {entry['path'] for entry in node.files_list()}
    # writes and commits would perturb work a resume expects intact
    with pytest.raises(RuntimeError, match='paused'):
        node.files_write('inputs/late.txt', b'x')
    with pytest.raises(RuntimeError, match='paused'):
        node.files_commit(['inputs/late.txt'], 'late')
    # a resumed (idle) node accepts the same write
    node.status_set('idle')
    assert node.files_write('inputs/late.txt', b'x')['size'] == 1
