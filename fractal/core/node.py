...
class Node:
    ...
    def reset(path: pathlib.Path) -> str:
        ...
        and while any node is paused: a parked worktree holds frozen
        uncommitted mid-step work that no session-liveness check can see.
        ...
        # refuse over frozen work -- a paused node has no session for
        # reset.sh's liveness refusal to catch (the script re-checks .status
        # under the lock; this pre-check is the friendly early error)
        if node.exists():
            paused = [row['node'] for row, _ in node._live_descendants(status='paused')]
            if paused:
                names = ', '.join(paused)
                raise RuntimeError(
                    f'Cannot reset: paused node(s) hold frozen work'
                    f' ({names}). Resume or kill them first.'
                )
        # refuse if the caller stands inside a node worktree -- git cannot
        # remove a worktree the caller occupies
        cwd = pathlib.Path.cwd().resolve()
        for worktree_path in _worktree_map(repo_dir).values():
            worktree_dir = pathlib.Path(worktree_path).resolve()
        ...
        if node.exists():
            for _, descendant in node._live_descendants(status='active'):
                descendant._reconcile_status()
        # run reset.sh under the .worktrees flock so its worktree remove/prune
        # does not race a concurrent init/delete (the same lock child_add takes) --
        # but only when .worktrees exists; creating it would defeat reset.sh's
        # nothing-to-reset check, which keys off that directory
        worktrees = repo_dir / '.worktrees'
        if worktrees.is_dir():
            with open(worktrees / '.lock', 'a', encoding='utf-8') as lock_file:
                fcntl.flock(lock_file, fcntl.LOCK_EX)
                result = node._run_script('reset.sh', f'{path}')
        else:
            result = node._run_script('reset.sh', f'{path}')
        ...
        for root_branch in roots:
            ...
            Node._deregister_subtree(
                node.db, repo_dir, root_branch, descendant_branches
            )
    ...
    def files_list(
        ...
        *,
        ...
        if since is not None:
            ...
            out = _git(['ls-files', '-z'], cwd=self._root, check=False) or ''
        ...
        for rel in candidates:
            # skip fractal machinery: any .fractal component (sibling
            # projects' committed seeds included) and the project's wiki
            if '.fractal' in rel.casefold().split('/'):
                continue
            ...
            if entry.get('change') != 'deleted':
                ...
                try:
                    ...
                    if abs_path.is_symlink() and not (
                        abs_path.resolve().is_relative_to(self._root)
                    ):
                        continue
    ...
    def files_read(
        ...
        *,
        ...
        Returns:
            ...
            (callers download it via :meth:`files_path` instead).
        ...
        tracked = _git(cmd, cwd=self._root, check=False)
        ...
        if before:
            ...
            try:
                if abs_path.is_file() and abs_path.resolve().is_relative_to(self._root):
    ...
    def files_path(self: Node, path: str) -> pathlib.Path:
        """Resolve a project file to its on-disk path (validated).

        The download side of :meth:`files_read` -- same validation, tracked
        ...
        if (
            ...
            or not abs_path.resolve().is_relative_to(self._root)
        ):
            raise ValueError(f'Not a readable project file: {path!r}')
        return abs_path
    ...
    def files_write(self: Node, path: str, data: bytes) -> dict[str, Any]:
        ...
        only once committed -- via :meth:`files_commit`, promptly: on a scoped
        ...
        norm = self._validate_relpath(path)
        # atomic replace via a unique temp + os.replace, so a concurrent
        # download never streams a half-written upload
        abs_path = self._root / norm
    ...
    def files_commit(self: Node, paths: list[str], message: str) -> dict[str, Any]:
        ...
        msg = f'{self._branch}: files ({message})'
        _git(['commit', '--no-verify', '-m', msg, '--', *norm], cwd=self._root)
        sha = _git(['rev-parse', 'HEAD'], cwd=self._root)
    ...
    def files_archive(
        ...
        *,
        ...
        Read-only: zips the :meth:`files_list` set (full or changed); the
        ...
        Args:
            path: Restrict to a worktree-relative subtree; all files if
                ``None``.
            since: Archive the changed set instead (see :meth:`files_list`).
        ...
        with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as archive:
            for entry in self.files_list(path=path, since=since):
    ...
    def _diff_anchor(self: Node, since: str) -> Optional[str]:
        ...
        if since == 'commit':
            ...
            return _git(cmd, cwd=self._root, check=False) or None
        branch = self._branch
        # the current incarnation's floor: history rows persist across delete
        # and reset, so a re-inited branch name must not anchor on a dead
        # namesake's events
        floor = (
        ...
        if since == 'base':
            ...
            query = (
                ...
                " AND metadata != '' ORDER BY event_id DESC LIMIT 1"
            )
            ...
            base = self.config_get('base') or branch.rpartition('.')[0]
            if not base:
                return None
    ...
    def _validate_relpath(self: Node, path: str) -> str:
    ...
    def session_transcript(self: Node, agent: str, session: str) -> dict[str, Any]:
        """Read an agent session's transcript for this node.

        Both agents persist a session as a JSONL file that grows while the
        session runs, so polling this returns the live transcript. Claude
        keys its transcripts by the agent's cwd -- the worktree, for loop
        steps and chat forks alike -- under the *user's* config home
        (``$CLAUDE_CONFIG_DIR``, else ``~/.claude``), which fractal does not
        relocate; codex rollouts live under the node's own ``CODEX_HOME``
        (``<node_dir>/.codex``). ``path`` rides along so a caller can tail or
        range-read the file as it grows -- for claude it is the expected
        location even while absent, so a poller can wait for it to appear.
        ...
        if agent == 'claude':
            ...
            # non-alphanumeric character dashed; the config home is the
            # user's own
            home = os.environ.get('CLAUDE_CONFIG_DIR')
            config_dir = pathlib.Path(home) if home else pathlib.Path.home() / '.claude'
            slug = re.sub(r'[^A-Za-z0-9]', '-', str(self._root))
            transcript = config_dir / 'projects' / slug / f'{session}.jsonl'
            if not transcript.is_file():
                # the slug rule is claude's own contract -- cover drift with a
                # config-home-wide glob, gated on the id being recorded for
                # THIS node: an ungated glob would serve any session of the
                # OS user, from any project
                ...
                params = (self._branch, session, self._branch, session)
                if found and self.db.read(query=query, params=params):
                    transcript = found[-1]
        elif agent == 'codex':
            # codex rollouts nest by date under the node's own codex home
            sessions_dir = self._node_dir / '.codex' / 'sessions'
...
def _git_bytes(
    ...
    *,
    ...
    The binary-safe, unstripped, non-raising counterpart to :func:`_git`, for
    reading a file blob (``git show <ref>:<path>``) where text decoding and
    whitespace stripping would corrupt content -- a non-zero exit (e.g. the
    path absent at that revision) degrades to ``None``.
